<x-app-layout>

    <main class="h-auto p-4 pt-20 md:ml-64">
        <div class="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div class="flex h-32 flex-col justify-center gap-4 rounded-lg border-2 border-white bg-white p-10 md:h-52">
                <div class="">
                    <svg class="size-10" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                        stroke-width="1.5" stroke="currentColor" class="size-6">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M2.25 18.75a60.07 60.07 0 0 1 15.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 0 1 3 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 0 0-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 0 1-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 0 0 3 15h-.75M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm3 0h.008v.008H18V10.5Zm-12 0h.008v.008H6V10.5Z" />
                    </svg>
                </div>
                <div class="text-3xl font-bold"> {{ Number::currency($donations->sum('amount'), 'GHS') }}</div>
                <div class="text-gray-500">Total Amount of Donations</div>
            </div>


            <div class="flex h-32 flex-col justify-center gap-4 rounded-lg border-2 border-white bg-white p-10 md:h-52">
                <div class="">
                    <svg class="size-10" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24"
                        height="24" stroke="currentColor" fill="none">
                        <path
                            d="M2 12C2 8.46252 2 6.69377 3.0528 5.5129C3.22119 5.32403 3.40678 5.14935 3.60746 4.99087C4.86213 4 6.74142 4 10.5 4H13.5C17.2586 4 19.1379 4 20.3925 4.99087C20.5932 5.14935 20.7788 5.32403 20.9472 5.5129C22 6.69377 22 8.46252 22 12C22 15.5375 22 17.3062 20.9472 18.4871C20.7788 18.676 20.5932 18.8506 20.3925 19.0091C19.1379 20 17.2586 20 13.5 20H10.5C6.74142 20 4.86213 20 3.60746 19.0091C3.40678 18.8506 3.22119 18.676 3.0528 18.4871C2 17.3062 2 15.5375 2 12Z"
                            stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                        <path
                            d="M14.551 12C14.551 13.3807 13.4317 14.5 12.051 14.5C10.6703 14.5 9.55099 13.3807 9.55099 12C9.55099 10.6193 10.6703 9.5 12.051 9.5C13.4317 9.5 14.551 10.6193 14.551 12Z"
                            stroke="currentColor" stroke-width="1.5" />
                        <path d="M5 12L6 12" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" />
                        <path d="M18 12L19 12" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" />
                    </svg>

                </div>
                <div class="text-3xl font-bold"> {{ Number::currency($contributions->sum('amount'), 'GHS') }}</div>
                <div class="text-gray-500">Total Amount of Funeral Wages</div>
            </div>
            <div class="flex h-32 flex-col justify-center gap-4 rounded-lg border-2 border-white bg-white p-10 md:h-52">
                <div class="">
                    <svg class="size-10" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24"
                        height="24" stroke="currentColor" fill="none">
                        <path
                            d="M18.6161 20H19.1063C20.2561 20 21.1707 19.4761 21.9919 18.7436C24.078 16.8826 19.1741 15 17.5 15M15.5 5.06877C15.7271 5.02373 15.9629 5 16.2048 5C18.0247 5 19.5 6.34315 19.5 8C19.5 9.65685 18.0247 11 16.2048 11C15.9629 11 15.7271 10.9763 15.5 10.9312"
                            stroke="currentColor" stroke-width="1.5" stroke-linecap="round" />
                        <path
                            d="M4.48131 16.1112C3.30234 16.743 0.211137 18.0331 2.09388 19.6474C3.01359 20.436 4.03791 21 5.32572 21H12.6743C13.9621 21 14.9864 20.436 15.9061 19.6474C17.7889 18.0331 14.6977 16.743 13.5187 16.1112C10.754 14.6296 7.24599 14.6296 4.48131 16.1112Z"
                            stroke="currentColor" stroke-width="1.5" />
                        <path
                            d="M13 7.5C13 9.70914 11.2091 11.5 9 11.5C6.79086 11.5 5 9.70914 5 7.5C5 5.29086 6.79086 3.5 9 3.5C11.2091 3.5 13 5.29086 13 7.5Z"
                            stroke="currentColor" stroke-width="1.5" />
                    </svg>

                </div>
                <div class="text-3xl font-bold">{{ $members->count() }}</div>
                <div class="text-gray-500">Total Number of Community Members</div>
            </div>
            <div class="flex h-32 flex-col justify-center gap-4 rounded-lg border-2 border-white bg-white p-10 md:h-52">
                <div class="">
                    <svg class="size-10" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24"
                        height="24" stroke="currentColor" fill="none">
                        <path
                            d="M2 12C2 8.46252 2 6.69377 3.0528 5.5129C3.22119 5.32403 3.40678 5.14935 3.60746 4.99087C4.86213 4 6.74142 4 10.5 4H13.5C17.2586 4 19.1379 4 20.3925 4.99087C20.5932 5.14935 20.7788 5.32403 20.9472 5.5129C22 6.69377 22 8.46252 22 12C22 15.5375 22 17.3062 20.9472 18.4871C20.7788 18.676 20.5932 18.8506 20.3925 19.0091C19.1379 20 17.2586 20 13.5 20H10.5C6.74142 20 4.86213 20 3.60746 19.0091C3.40678 18.8506 3.22119 18.676 3.0528 18.4871C2 17.3062 2 15.5375 2 12Z"
                            stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                        <path
                            d="M14.551 12C14.551 13.3807 13.4317 14.5 12.051 14.5C10.6703 14.5 9.55099 13.3807 9.55099 12C9.55099 10.6193 10.6703 9.5 12.051 9.5C13.4317 9.5 14.551 10.6193 14.551 12Z"
                            stroke="currentColor" stroke-width="1.5" />
                        <path d="M5 12L6 12" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" />
                        <path d="M18 12L19 12" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" />
                    </svg>
                </div>
                <div class="text-3xl font-bold"> {{ $users->count() }}</div>
                <div class="text-gray-500">Total Amount Users</div>
            </div>

        </div>


        <div class="my-4 w-full max-w-md rounded-lg bg-white p-4 dark:border-gray-700 dark:bg-gray-800 sm:p-8">
            <div class="mb-4 flex items-center justify-between">
                <h5 class="text-xl font-bold leading-none text-gray-900 dark:text-white">Community Members</h5>
                <a href="{{ route('members') }}"
                    class="text-sm font-medium text-blue-600 hover:underline dark:text-blue-500">
                    View all
                </a>
            </div>
            <div class="flow-root">
                <ul id="communityMember" role="list" class="divide-y divide-gray-200 dark:divide-gray-700">
                    @foreach ($members->take(5) as $member)
                        <li class="py-3 sm:py-4">
                            <a href="{{ route('member.single', $member->id) }}">
                                <div class="grid grid-cols-8">
                                    <div class="col-span-6 min-w-0 flex-1">
                                        <p class="truncate text-sm font-medium text-gray-900 dark:text-white">
                                            {{ $member->name }}
                                        </p>
                                        <p class="truncate text-sm text-gray-500 dark:text-gray-400">
                                            {{ $member->phone_number }}
                                        </p>
                                    </div>
                                    <div
                                        class="col-span-2 inline-flex items-center text-base text-sm font-semibold text-gray-900 dark:text-white">
                                        {{ $member->created_at->toFormattedDateString() }}
                                    </div>
                                </div>
                            </a>
                        </li>
                    @endforeach
                </ul>
            </div>
        </div>

    </main>
</x-app-layout>


<script>
    const communityMember = {
        name: 'Ohene Adjei',
        email: 'email@ohene.com',
        balance: 320,
    }
</script>
